...
//      Map<String, String> oldAttributes = substrate.getAttributes(node);
  ...
  public class NonInvertibleCursor implements NindoCursor {
    ...
    private void begin2() {
      beginChange();
      builder = new AnnotationsNormalizer<BufferedDocOp>(new DocOpBuffer());
      sizeDiffSoFar = 0;
  ...
  public String toString() {
    String debugXml;
    try {
      debugXml = DocOpUtil.toXmlString(asOperation());
    } catch (RuntimeException e2) {
      try {
        debugXml = "(NO ANNOTATIONS SHOWN) " + XmlStringBuilder.outerXml(this);
      } catch (RuntimeException e3) {
        debugXml = "Tried twice to get the goddamn xml and failed - it's just that broken";
      }
    }
    return debugXml;
  }
  ...
  public DocInitialization asOperation() {
    ...
    try {
      BufferedDocInitialization initialisation = DocOpUtil.asInitialization(
          Composer.compose(domOp, annotationsOp));
